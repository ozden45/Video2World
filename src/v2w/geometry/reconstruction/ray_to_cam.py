...
def reconstruct_ray_to_cam(ray_pts: RayPoints, depth: torch.Tensor) -> CameraPoints:

    r = ray_pts.coords

    cam_coords = depth.unsqueeze(-1) * r

    J = depth.view(-1,1,1) * torch.eye(3, device=r.device, dtype=r.dtype)
...
def reconstruct_ray_to_cam_batched(ray_pts: RayPoints, depth: torch.Tensor):
    ...
    J = depth.unsqueeze(-1).unsqueeze(-1) * I
